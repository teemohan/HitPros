      ...
      new Vue({
        ...
        methods: {
          ...
          async getSelectionAttrs() {
            try {
              const res = await kkAjax.get(`/spu/selection-attrs/other?sku=${skypdp.sku}`);
              if (res.code == 200 && res.data) {
                this.selectionAttrs = (res.data.selectionAttrs || []).filter(item => item.attrName != 'match');
                this.availableAttrs = res.data.availableAttrs || [];
                const matchedProduct = this.availableAttrs.find(item => item.skuCode == skypdp.sku);
          ...
          calculateCurrentSkuAttrs() {
            ...
            if (currentSkuAttrs?.attrs) {
              this.selectedValues = [...currentSkuAttrs.attrs.slice(0, this.selectionAttrs.length)];
            }
          },
          getSelectOptions(attr, attrIndex) {
            if (!attr?.values) return [];
            if (attrIndex == 0) {
              return attr.values;
            }
            return this.getAvailableValuesForIndex(attrIndex);
          },
          getAvailableValuesForIndex(targetIndex) {
            ...
            this.availableAttrs.forEach(item => {
              ...
              for (let i = 0; i < targetIndex; i++) {
                if (currentSelections[i] && currentSelections[i] != '' && item.attrs[i] != currentSelections[i]) {
                  isCompatible = false;
                  break;
                }
              }
          ...
          onSelectChange(value, index) {
            ...
            // 检查当前组合是否仍然有效
            const isCurrentCombinationValid = this.availableAttrs.some(item => {
              if (!item.attrs || item.attrs.length < this.selectionAttrs.length) return false;
              // 检查所有已选择的属性是否匹配
              for (let i = 0; i < this.selectionAttrs.length; i++) {
                if (this.selectedValues[i] && this.selectedValues[i] !== '' && item.attrs[i] !== this.selectedValues[i]) {
                  return false;
                }
              }
              return true;
            ...
            if (!isCurrentCombinationValid) {
              for (let i = index + 1; i < this.selectionAttrs.length; i++) {

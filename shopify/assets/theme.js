$(function () {
  ...
  var SafeStickyHeader = class extends HTMLElement {
    connectedCallback() {
      this.lastKnownY = window.scrollY;
      this.currentTop = 0;
      this.hasPendingRaf = false;
    ...
    _handleScrollHide() {
      ...
      const headerHeight = storeHeader ? storeHeader.clientHeight : this.clientHeight;
      if (scrollDifference > 0 && currentScrollY > headerHeight) {
        if (!this.isScrollHidden) {
  ...
  window.customElements.define('safe-sticky-header', SafeStickyHeader);
...
`;
  ...
  var FlickityCarousel = class extends CustomHTMLElement {
    constructor() {
      ...
      // 修改条件判断，允许单个图片元素也能初始化
      if (this.childElementCount === 1 && !this.querySelector('.product__media-item[data-media-type="image"]')) {
        return;
      }
    ...
    async connectedCallback() {
      this.delegate.on("click", '[data-action="prev"]', (event) => this.previous());
      this.delegate.on("click", '[data-action="next"]', (event) => this.next());
    }
  ...
  var FlickityControls = class extends CustomHTMLElement {
    ...
    async _onSlideChanged(animate = true) {
      ...
      const $prevButtons = $(this).find('button[data-action="prev"]');
      const $nextButtons = $(this).find('button[data-action="next"]');
      ...
      } else if (flickityInstance.selectedIndex == flickityInstance.cells.length - 1) {
        $nextButtons
          .css({
            background: 'unset',
            border: 'none',
  ...
  function computed_price(discountJson, basePrice, quantity) {
    let finalPrice;
    if (discountJson != '[]') {
  ...
  var QuantitySelector = class extends CustomHTMLElement {
    ...
    highlightPriceRange(quantity) {
      ...
      const priceItems = document.querySelectorAll('.ladder-price-text');

      priceItems.forEach((item, index) => {
        const quantityText = item.textContent.trim();
        const quantityMatch = quantityText.match(/\d+/g);

        if (quantityMatch && quantityMatch.length === 2) {
          const minQuantity = parseInt(quantityMatch[0]);
          const maxQuantity = parseInt(quantityMatch[1]);

          if (!isNaN(inputValue) && inputValue >= minQuantity && inputValue <= maxQuantity) {
            item.parentElement.classList.add('active');
          } else {
            item.parentElement.classList.remove('active');
          }
        } else if (index === priceItems.length - 1 && quantityText.includes('>=')) {
          const minQuantity = parseInt(quantityText.replace('>=', '').trim());

          if (!isNaN(inputValue) && inputValue >= minQuantity) {
            item.parentElement.classList.add('active');

            var isHidden = $(item).siblings().text().includes('Please inquire');
            if (isHidden) {
              $('#inquiries-btn').prop('disabled', false);
              $('.buy-btn-box #AddToCart').hide();
            } else {
              $('.buy-btn-box #AddToCart').show();
            }
          } else {
            $('#inquiries-btn').prop('disabled', true);
            item.parentElement.classList.remove('active');
            $('.buy-btn-box #AddToCart').show();
          }
        }
    ...
    computedFirst() {
      ...
      if (basePrice) {
        basePrice = parseFloat(basePrice.replace(/,/g, ''));
        const finalPrice = computed_price(discountJson, basePrice, moqValue);
        $('.cus-product .subtotal .computed_price').text(`${finalPrice}`);
        // $('.cus-product .product-sticky-form__price').text(`${finalPrice}`);
      }
    }
  ...
  var ProductQuantitySelector = class extends CustomHTMLElement {
    get $hiddenQuantity() {
  ...
  function cartDebounce(func, wait) {
    let timeout;
  ...
  const cartInput = cartDebounce((updates) => {
    $('#cart-checkout-button').attr('data-disabled', '1');
  ...
  var HeaderSearchForm = class extends HTMLElement {
    connectedCallback() {
      this.initVue();
    }
    initVue() {
      const $formWrapper = $(this).closest('.header-search-form-wrapper');
      new Vue({
        el: `#${$(this).find('.header-search-form').attr('id')}`,
        delimiters: ['${', '}'],
        data() {
          return {
            restaurants: [],
        ...
        mounted() {
          ...
          this.searchStr = urlParams.get('q');
        ...
        methods: {
          ...
          handleDeleteHistory(i) {
            ...
            const idx = searchHistory.findIndex(item => item.value == i.value);
            if (idx !== -1) {
              searchHistory.splice(idx, 1);
              localStorage.setItem('searchHistory', JSON.stringify(searchHistory));
              this.cb(searchHistory);
            }
          },
          async querySearchAsync(searchStr, cb) {
            let results;
            if (!searchStr?.trim()) {
              results = JSON.parse(localStorage.getItem('searchHistory')) || [];
            } else {
              try {
                const formData = {
                  keyword: searchStr,
          ...
          storageHistory(confirmStr) {
            ...
            if (confirmStr && !searchHistory.find(item => item.value === confirmStr)) {
              searchHistory.unshift({ value: confirmStr });
              localStorage.setItem('searchHistory', JSON.stringify(searchHistory));
            }
          },
          async getResult() {
  ...
  var StockLabel = class extends HTMLElement {
    connectedCallback() {
      try {
    ...
    setStcokText(demandNum) {
      const positiveIntegerRegex = /^[1-9]\d*$/;
      if (!positiveIntegerRegex.test(demandNum)) demandNum = 1;
      let $stockText;
      if (this.stockNum <= 0) {
        $stockText = $('<span class="coming-stock stock-tag fb-max-sm:text-12">Restocking</span>');
      } else if (this.stockNum >= +demandNum) {
        $stockText = $('<span class="instock-ready stock-tag fb-max-sm:text-12 ">In Stock</span>');
      } else {
        $stockText = $('<span class="instock-partial stock-tag fb-max-sm:text-12">In Stock</span>');
      }
      $(this).html($stockText);
    }
    get stockNum() {
  ...
  var RecentlyViewedProducts = class extends HTMLElement {
    async connectedCallback() {
      if (this.searchQueryString === '') {
        ...
        $('.shopify-section--recently-viewed-products').show();
      }
      const response = await fetch(
  ...
  var BlogPostNavigation = class extends HTMLElement {
    ...
    _updateProgressBar() {
      const storeHeader = document.querySelector('store-header');
      const isHeaderHidden = storeHeader && storeHeader.classList.contains('header--scroll-hidden');
      const stickyHeaderOffset = storeHeader ? storeHeader.offsetHeight : getStickyHeaderOffset();
      const marginCompensation = window.matchMedia(window.themeVariables.breakpoints.pocket).matches ? 40 : 80,
      ...
      if (isHeaderHidden) {
        this.classList.add('nav--sticky');
        this.style.top = '0px';
      } else {
        this.classList.remove('nav--sticky');
        this.style.top = `${stickyHeaderOffset}px`;
      }
  ...
  var StoreHeader = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      // index add homepage
      // if (this._isNoPage()) {
      this._onScrollHideListener = throttle(this._handleScrollHide.bind(this), 16);
      window.addEventListener('scroll', this._onScrollHideListener);
      // }
    }
    disconnectedCallback() {
      super.disconnectedCallback();
      ...
      // remove index scoll
      if (this._onScrollHideListener) {
        window.removeEventListener('scroll', this._onScrollHideListener);
      }
    }
    get isSticky() {
    ...
    _handleScrollHide() {
      ...
      if (scrollDifference > 0 && currentScrollY > this.clientHeight) {
        if (!this.isScrollHidden) {
          this.isScrollHidden = true;
          this.classList.add('header--scroll-hidden');
          const sectionHeader = this.closest('#shopify-section-header');
          if (sectionHeader) {
            sectionHeader.classList.add('header--zindex');
          }
        }
      }
      else if (scrollDifference < 0) {
        if (this.isScrollHidden) {
          this.isScrollHidden = false;
          this.classList.remove('header--scroll-hidden');
          const sectionHeader = this.closest('#shopify-section-header');
          if (sectionHeader) {
            sectionHeader.classList.remove('header--zindex');
          }
        }
      }
      this.lastScrollY = currentScrollY;
  ...
  var ProductForm = class extends HTMLFormElement {
    ...
    async _onSubmit(event) {
      ...
      const formData = {
        items: [
          {
            ...this.formDataToObject(productForm),
          }
        ]
      };
      ...
      new Vue({
        ...
        methods: {
          getButtonClasses(attrIndex, value) {
            ...
            return {
              'border-6B84B3 text-white font-medium bg-main fb-md:font-bold': isSelected,
  ...
  var ProductOverview = class extends CustomHTMLElement {
    async connectedCallback() {
      this.rootDelegate.on('build-product-overview', this.renderOverview.bind(this));
    }
    renderOverview({ detail }) {
      $(this).find('.product-desc').text(detail.description);
      if (!$(this).find('.product-tags').data('applicable-scene')) {
        $(this).find('.product-tags').hide();
      }
    }
  };
  window.customElements.define('product-overview', ProductOverview);
  var ProductInformation = class extends CustomHTMLElement {
    async connectedCallback() {
      this.isGeneralInfoExistBlankValue();
      this.rootDelegate.on('build-product-information', this.renderInformation.bind(this));
    ...
    isGeneralInfoExistBlankValue() {
      let isAllBlankValue = true;
      const $generalInformation = $(this).find('.general-information');
      $generalInformation.find('.value').each(function () {
        if ($(this).text().trim() != '') {
          isAllBlankValue = false;
        }
      });
      if (isAllBlankValue) {
        $generalInformation.attr('hide', true);
        ...
        if (!keys || keys.length === 0) {
          switch (itemKey) {
            case 'weightDimensions':
              ...
              $(this).find('.weight-dimensions').hide();
              break;
            case 'specifications':
              $(this).find('.specifications').attr('hide', true);
              $(this).find('.specifications').hide();
              break;
            case 'complianceCertificate':
              $(this).find('.compliance-certificates').attr('hide', true);
              $(this).find('.compliance-certificates').hide();
              break;
          }
        } else {
          let $productInfoLine;
          switch (itemKey) {
            case 'weightDimensions':
              $productInfoLine = $(this).find('.weight-dimensions');
              break;
            case 'specifications':
              $productInfoLine = $(this).find('.specifications');
              break;
            case 'complianceCertificate':
              $productInfoLine = $(this).find('.compliance-certificates');
              break;
          }
          keys.forEach((key) => {
            let wrappedValue
  ...
  window.customElements.define('product-information', ProductInformation);
  var MoreProductMedia = class extends CustomHTMLElement {
    async connectedCallback() {
      this.rootDelegate.on('build-product-media', this.renderMoreMedia.bind(this));
    }
    renderMoreMedia({ detail }) {
      if (detail.detailImages) {
        detail.detailImages.forEach((img) => {
          const $img = $(`<img src="${img.src}" title="${img.title}" />`);
          $(this).find('.product-img-list').append($img);
        });
      }
      if ($(this).data('exist-video')) {
        $(this)
          .find('.img')
          .click(() => {
            $(this).find('.video-container').addClass('show-video');
            $(this).find('video').get(0).play();
          });
      }
      if (!detail.detailImages && !$(this).data('exist-video')) $(this).closest('.product-media-detail-wrapper').hide();
    }
  };
  ...
  window.customElements.define('product-search-facet', ProductSearchFacet);
  ...
  var FacetFilters = class extends DrawerContent {
    connectedCallback() {
      ...
      if (this.alwaysVisible) {
        this.matchMedia = window.matchMedia('screen and (max-width: 1199px)');
  ...
  var CartCount = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.rootDelegate.on('cart:updated', (event) => {
        ...
        if (+event.detail.cart['item_count'] > 99) {
          cart_count = '99+';
        } else {
          cart_count = event.detail.cart['item_count'];
        }
  ...
  var OpenableElementCart = class extends CustomHTMLElement {
    static get observedAttributes() {
      return ['open'];
    }
    constructor() {
      ...
      return (this._focusTrap =
        ...
        createFocusTrap(this, {
          ...
          initialFocus: this.hasAttribute('initial-focus-selector')
            ? this.getAttribute('initial-focus-selector')
            : void 0,
          clickOutsideDeactivates: window.matchMedia('(max-width: 1199px)').matches ? (event) =>
            !(event.target.hasAttribute('aria-controls') && event.target.getAttribute('aria-controls') === this.id) : false,
  ...
  window.customElements.define('openable-element-cart', OpenableElementCart);
  var DrawerContentCart = class extends OpenableElementCart {
    connectedCallback() {
      super.connectedCallback();
      if (this.hasAttribute('reverse-breakpoint')) {
        this.originalDirection = this.classList.contains('drawer--from-left') ? 'left' : 'right';
  ...
  window.customElements.define('drawer-content-cart', DrawerContentCart);
  var CartDrawer = class extends DrawerContentCart {
    connectedCallback() {
      super.connectedCallback();
    ...
    async _rerenderCart(event) {
      ...
      setTimeout(async () => {
        var _a2;
        // 添加安全检查，避免 scrollTop 错误
        const drawerContent = this.querySelector('.js-drawer__content');
        const previousPosition = drawerContent ? drawerContent.scrollTop : 0;

        if (cartContent && cartContent['item_count'] === 0) {
          const animation = new CustomAnimation(
            new ParallelEffect(
              Array.from(this.querySelectorAll('.js-drawer__content, .drawer__footer')).map((item) => {
  ...
  var LineItemQuantity = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.delegate.on('blur', 'input', this._onQuantityMaxChanged.bind(this));
    }
    ...
    _onQuantityMaxChanged(event, target) {
      event.preventDefault();
      if ($(target).val() == 1000000) {
        this._onQuantityChanged(event, target);
      }
    }

(() => {
          ...
          async querySearchAsync(searchStr, cb) {
            ...
            if (!searchStr?.trim()) {
              ...
              try {
                ...
                const response = await fetch('https://doc-shop-uat.northskysupply.com/web/openapi/suggest', {
                  method: 'POST',
    ...
    initVue() {
      new Vue({
        ...
        methods: {
          ...
          resetData() {
            this.hoverMenu1.isSelected = this.hoverMenu2.isSelected = this.hoverMenu3.isSelected = false;
            this.hoverMenu1 = this.hoverMenu2 = this.hoverMenu3 = {};
            this.menu2 = this.menu3 = [];
          ...
          async getAllList() {
            try {
              const response = await fetch('https://doc-shop-uat.northskysupply.com/web/openapi/adlink/product/collection/tree', {
  ...
  window.customElements.define('pc-menu-sidebar', pcMenuSidebar);

  var MobileMenuSidebar = class extends HTMLElement {
    connectedCallback() {
      this.initVue();
    }
    initVue() {
  ...
  window.customElements.define('mobile-menu-sidebar', MobileMenuSidebar);
  ...
  var StockLabel = class extends HTMLElement {
    ...
    setStcokText(demandNum) {
      ...
      let $stockText;
      if (this.stockNum <= 0) {
        $stockText = $('<span class="coming-stock stock-tag">Restocking</span>');
      } else if (this.stockNum >= +demandNum) {
        $stockText = $('<span class="instock-ready stock-tag">In Stock</span>');
      } else {
        $stockText = $('<span class="instock-partial stock-tag">In Stock</span>');
      }
      $(this).html($stockText);
    }
    get stockNum() {
  ...
  var ProductAttrs = class extends CustomHTMLElement {
      ...
      $(this)
        ...
        .click(async function () {
          ...
          try {
            ...
            const response = await fetch('https://doc-shop-uat.northskysupply.com/web/product/select-attrs', {
              method: 'POST',
              body: JSON.stringify(formData),
              headers: {
                'Content-Type': 'application/json',
    ...
    async getProductAttrs() {
      try {
        const response = await fetch(`https://doc-shop-uat.northskysupply.com/web/spu/selection-attrs?sku=${this.sku}`);
        const res = await response.json();
        if (res.code != 200) {
          window.vue_message.$message({
            showClose: true,
  ...
  var ProductItem = class extends CustomHTMLElement {
    ...
    async handleClickFavoritebutton(e) {
      ...
      try {
        const response = await fetch('https://doc-shop-uat.northskysupply.com/web/wish/' + (status == 1 ? 'clear' : 'save'), {
          method: 'POST',
          body: JSON.stringify(formData),
          headers: {
            'Content-Type': 'application/json',
    ...
    async fetchAndSetFavoriteStatus() {
      ...
      try {
        const response = await fetch('https://doc-shop-uat.northskysupply.com/web/wish/select', {
          method: 'POST',
          body: JSON.stringify(formData),
          headers: {
            'Content-Type': 'application/json',
...
// 改造factspu功能
  ...
  var CartCount = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      if(email) {
        const cartRes = await fetch(`https://doc-shop-uat.northskysupply.com/web/shopping/cart?email=${email}`);
      ...
      this.rootDelegate.on('cart:updated', (event) => {
        ...
        if(email) {
          ...
          fetch('https://doc-shop-uat.northskysupply.com/web/shopping/cart/save ', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
